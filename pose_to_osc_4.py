...
def video_callback(dev, data, timestamp):
    global latest_rgb
    # data приходит как numpy array (H, W, 3) в RGB
    latest_rgb = cv2.cvtColor(data, cv2.COLOR_RGB2BGR)


def depth_callback(dev, data, timestamp):
    global latest_depth
    # data приходит как numpy array (H, W) uint16, в мм (т.к. depth_format=DEPTH_MM)
...
try:
    while True:
        # прокачиваем события USB -- это вызывает video_callback/depth_callback
        freenect.process_events(ctx)

        if latest_rgb is None or latest_depth is None:
            continue  # ещё не пришёл первый кадр
